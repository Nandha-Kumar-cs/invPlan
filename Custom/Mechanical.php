...
CONCAT('<p1>',datediff(potable.po_rx_date, NOW()),'days ','</p1>')

                                                 ELSE

                                                         
CONCAT(datediff(potable.po_rx_date, NOW()),'days')
...
{
			 ?>
				 <td style="width:420px;border-bottom: 1px solid #ffffff;"><b><?php echo $product_result;?>...</b></td>
			<?php 
			 }
			 else{ ?>
...
//$total_pen=sum($val_data);
	...
	echo '<tr><td style="border-top: 1px solid #ffffff;">Total pending - '.$total_pen.'</td>'.$val_data.'</tr>';
$val_data='';	
$total_pen=0;
?>
</tr>	
...
$total_pen+=$row["pending_qty"];
    ...
    <td style="width:75px;"><b><?php echo $row["val"];?></b></td>
