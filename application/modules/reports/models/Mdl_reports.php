        ...
        (
            SELECT SUM(invoice_balance) FROM ip_invoice_amounts 
                ...
                (
                    SELECT invoice_id FROM ip_invoices
                        WHERE ip_invoices.client_id = ip_clients.client_id 
                            AND invoice_date_created <= DATE_SUB(NOW(),INTERVAL 1 DAY) 
                            AND invoice_date_created >= DATE_SUB(NOW(), INTERVAL 60 DAY)
                )
        ) AS range_1', false);
            ...
            SELECT SUM(invoice_balance) FROM ip_invoice_amounts 
                ...
                (
                    SELECT invoice_id FROM ip_invoices
                        WHERE ip_invoices.client_id = ip_clients.client_id 
                            AND invoice_date_created <= DATE_SUB(NOW(),INTERVAL 60 DAY) 
                            AND invoice_date_created >= DATE_SUB(NOW(), INTERVAL 90 DAY)
                )
        ) AS range_2', false);
        ...
        (
            SELECT SUM(invoice_balance) FROM ip_invoice_amounts 
                ...
                (
                    SELECT invoice_id FROM ip_invoices
                        WHERE ip_invoices.client_id = ip_clients.client_id 
                            AND invoice_date_created <= DATE_SUB(NOW(),INTERVAL 91 DAY)
                )
        ) AS range_3', false);
