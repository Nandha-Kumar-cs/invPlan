...
function generate_invoice_pdf1($invoice_id, $stream = true, $invoice_template = null, $is_guest = null)
{
    $CI = &get_instance();
    ...
    $html = $CI->load->view('invoice_templates/pdf1/' . $invoice_template, $data, true);

    $CI->load->helper('mpdf');
    return pdf_create($html, trans('invoice') . '_' . str_replace(array('\\', '/'), '_', $invoice->invoice_number),
        $stream, $invoice->invoice_password, true, $is_guest, $include_zugferd, $associatedFiles,'L');
}
