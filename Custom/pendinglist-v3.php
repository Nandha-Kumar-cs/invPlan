...
concat(potable.pono,potable.lineno)
                                         ...
                                         ORDER BY val ASC, 
`product`.`product_name` ASC) as ft LEFT JOIN(SELECT ft.*,MIN(val) as max_val,SUM(pending_qty) as tot_pen_qty FROM (SELECT 
...
if($len>=500)				 
{
			 ...
			 <td style="width:50px; text-align:center;" rowspan="2"><b><?php echo $serial_no;?></b></td>
				 ...
				 <td style="width:50px;text-align:center;" rowspan="2"><b><?php echo $row["tot_pen_qty"];?></b></td>
				 <td><b>Days</b></td>
			<?php 
			 }
			 else{ ?>
...
$val_data.='<td style="width:40px;text-align:center;">'.$row["pending_qty"].'</td>';
